...
async function run() {
  ...
  // 1b) Old OTP storage: plaintext codes and a TTL on expiresAt (which would reset rate-limit counters).
  try {
    const otps = db.collection("otps");
    await otps.deleteMany({ code: { $exists: true } });
    for (const i of await otps.indexes()) {
      if (i.name === "expiresAt_1" || i.name === "mobile_1_code_1") await otps.dropIndex(i.name);
    }
  } catch (e) {
    if (e.codeName !== "NamespaceNotFound") console.warn("[migrate] otps:", e.message);
  }
