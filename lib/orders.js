...
/** Serialises checkouts per customer (works across processes, no transactions needed). */
async function withCheckoutLock(userId, fn) {
  const LOCK_MS = 30_000;
  for (let i = 0; i < 50; i++) {
    const now = new Date();
    const got = await User.updateOne(
      { _id: userId, $or: [{ checkoutLockUntil: { $exists: false } }, { checkoutLockUntil: null }, { checkoutLockUntil: { $lt: now } }] },
      { $set: { checkoutLockUntil: new Date(now.getTime() + LOCK_MS) } }
    );
    if (got.modifiedCount === 1) {
      try {
        return await fn();
      } finally {
        await User.updateOne({ _id: userId }, { $unset: { checkoutLockUntil: 1 } });
      }
    }
    await sleep(150);
  }
  throw new HttpError(429, "سفارش قبلی شما در حال پردازش است. چند لحظه دیگر دوباره تلاش کنید");
}

async function checkout(req) {
  if (!req.session.userId) throw new HttpError(401, "لطفاً ابتدا وارد حساب کاربری شوید");
  return withCheckoutLock(req.session.userId, () => checkoutLocked(req));
}
 ...
 */
async function checkoutLocked(req) {
