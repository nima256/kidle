...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // ایجاد سفارش با وضعیت "در حال پردازش" (بدون نیاز به پرداخت)
      const order = new Order({
        ...
        city,
        user: userId,
        products: productsForOrder.filter((p) => p !== null),
        delivery: delivery || "",
        originalPrice: subtotal,
        totalPrice: finalPrice,
        discount: appliedDiscount,
        discountAmount,
        status: "در حال پردازش",  // مستقیماً در حال پردازش
        paymentStatus: "پرداخت شده",  // فرض می‌کنیم پرداخت شده
        paymentMethod: "حضوری",  // یا هر روش دلخواه
      });
      ...
      // ذخیره شماره سفارش در سشن
      req.session.OrderNum = order.OrderNum;
      await req.session.save();

      // برگرداندن پاسخ موفقیت‌آمیز با شماره سفارش
      return res.json({
        success: true,
        message: "سفارش با موفقیت ثبت شد",
        orderId: order._id,
        ...
        total: finalPrice,
        discount: discountAmount,
        redirectUrl: "/api/order/payment-success"  // آدرس صفحه موفقیت
      });
